...
st.beta_set_page_config(page_title='Covid19 - Kenya',
                        page_icon= Image.open('cov.jpg'), layout='wide', initial_sidebar_state='auto')
...
data = pd.read_excel(r'C:\Users\hp\Desktop\covid19.xlsx')
worldwide = pd.read_csv(url)
...
summations = [data['Deaths'].sum(), data['Positive cases'].sum(), data['Recoveries'].sum()]
    
fatalityperc = summations[0]/summations[1]
recoveryperc = summations[2]/summations[1]
    
fatality_values =[summations[0], summations[1]-summations[0]]  
recovery_values =[summations[2], summations[1]-summations[2]]

data1 = pd.DataFrame(data.iloc[:,6:].stack())
data1.index.names = ['Date', 'County']
data1 = data1.rename(columns={0: "new cases"})
data1 = data1.reset_index()
data1 = data1.set_index('Date')

data2 = pd.DataFrame(data.iloc[:,:6]).fillna(0)
data2 = data2.reset_index()
data2['Week_Number'] = data2['Date'].dt.isocalendar().week
...
data3 = data2.drop(['Date','rate','Week_Number', 'month1'], axis=1).fillna(0)
...
with col1:
    ...
    axs[1,0].text(0.5,0.5, '{:,.0f}'.format(data['Positive cases'].sum()), fontsize=30, weight='black', ha='center', color='orange')
    axs[1,0].text(0.5,0.3, ' positive cases', fontsize=18, ha='center')
    axs[0,1].text(0.5,0.5, '{:,.0f}'.format(data['Recoveries'].sum()), fontsize=30, weight='heavy', ha='center', color='green')
    axs[0,1].text(0.5,0.3, ' recoveries', fontsize=18, ha='center')
    axs[1,1].text(0.5,0.5, '{:,.0f}'.format(data['Deaths'].sum()), fontsize=30, weight='extra bold', ha='center', color='red')
...
with col4:
    ...
    fig = px.line(data2, x='Date', y =['Positive cases','Recoveries','Deaths'], hover_data={'value':':,.0f','Date': False},
                 color_discrete_map={'Positive cases': '#0000FF',
                                     'Recoveries': '#00FF00',
                                     'Deaths':'#8B0000'})
...
with col8:
    st.markdown("<h2 style='text-align: center; color: green;'>Total cases, recoveries and fatalities</h2>",
                unsafe_allow_html=True)
    fig = px.line(data3, x='month',y = ['Positive cases','Recoveries','Deaths'],
                  hover_data={'value':':,.0f','month': False}, color_discrete_map={'Positive cases': '#0000FF',
                                                                                   'Recoveries': '#00FF00',
                                                                                   'Deaths':'#8B0000'})
    fig.layout.update(hovermode='x', yaxis=dict(title='Count', titlefont=dict(size=18), tickformat=',.0f',
                                                        visible=True, showgrid=False),
...
worldwide_cases = worldwide_cases.merge(codes, how='left')


# In[12]:


max_cases = worldwide_cases['Positive cases'].max()
Africa_cases = worldwide_cases[worldwide_cases['Continent'] == 'Africa']
max_cases_africa = Africa_cases['Positive cases'].max() 


# In[13]:


mid_point = worldwide_cases['Positive cases'].median()
mid_cases_africa = Africa_cases['Positive cases'].median()


# In[14]:


data5 = data1.reset_index()
data5['month1'] = data5['Date'].dt.month
data5['month'] = data5['month1'].apply(lambda x: calendar.month_abbr[x])
data5 = data5.drop(['Date'], axis=1)
data5 = data5.groupby(['month','County'], as_index=False)['new cases'].sum()
data5['month'] = pd.Categorical(data5['month'], categories=cats, ordered=True)
data5 = data5.sort_values(by=['month', 'new cases'], ascending=[True, False]).groupby('month').head(10)
data5['total cases'] = data5.groupby('County')['new cases'].transform('cumsum')
data5['total cases'] = data5['total cases'].astype(float).map("{:,.0f}".format)
...
with col10:
    ...
    fig = px.bar(data5, x='total cases', y='County', orientation='h', hover_name= None, color='County',
                 animation_frame='month', animation_group='County', range_x=[0,5000])
...
with col11:
    ...
    fig = px.choropleth(Africa_cases,locations='Country', locationmode='country names',
                        color='Positive cases', animation_frame='month', hover_name='Country',
                        hover_data = {'Positive cases':':,.0f','Country': False,
                                      'month': False, 'period': Africa_cases['Month_yr']},
                        animation_group='Positive cases', color_continuous_scale='reds',
...
fig = px.choropleth(worldwide_cases,locations='Country', locationmode='country names', color='Positive cases',
                    animation_frame='month', hover_name='Country', hover_data = {'Positive cases':':,.0f',
                                                                                 'Country':False,
                                                                                 'month':False,
